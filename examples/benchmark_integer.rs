//! Performance benchmark for the MLS-MPM simulation using integer atomics.
//!
//! This example is identical to `benchmark.rs` but demonstrates the performance
//! when using scaled integer atomics for Particle-to-Grid (P2G) accumulation.
//! This is useful for evaluating the overhead of fixed-point accumulation
//! compared to native floating-point atomics.
//!
//! The simulation executes 100 steps on a 1M particle, 1024x1024 grid setup.
...
fn main() -> Result<()> {
    ...
    // We use a scaling factor (1.0e8) for fixed-point representation.
    ...
    println!("Launching performance benchmark (1M particles, 1024x1024 grid, integer atomics)...");
    let start = Instant::now();
    for _ in (0..100).progress() {
        // Execute simulation step using the integer atomic pipeline.
        sim.launch()?;
    }
